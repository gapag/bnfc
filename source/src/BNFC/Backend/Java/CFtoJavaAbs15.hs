...
module BNFC.Backend.Java.CFtoJavaAbs15 (cf2JavaAbs) where
...
cf2JavaAbs :: TypeMapping -> String -> String -> CF -> [(FilePath, String)]
cf2JavaAbs tm _ packageAbsyn cf =
    concatMap (prData tm header packageAbsyn user) rules
 where
...
prData :: TypeMapping -> String -> String -> [UserDef] -> Data ->[(String, String)]
prData tm header packageAbsyn user (cat, rules) =
  categoryClass ++ mapMaybe (prRule tm header packageAbsyn funs user cat) rules
      where
      funs = map fst rules
      categoryClass
...
prRule tm h packageAbsyn funs user c (fun, cats)
  ...
  | otherwise = Just (fun, --a standard rule
    ...
    [
     ...
     render $ nest 2 $ vcat
        [ prInstVars vs
        , prConstructor tm fun user vs cats],
...
prConstructor tm c u vs cats =
  ...
  where
   (types, params) = unzip (prParams tm cats u (length cats) (length cats+1))
   interleave xs ys = hsep $ punctuate "," $ zipWith ((<+>) `on` text) xs ys

--Prints the parameters to the constructors.
prParams :: TypeMapping -> [Cat] -> [UserDef] -> Int -> Int -> [(String,String)]
prParams _ [] _ _ _ = []
prParams tm (c:cs) u n m = (tm (identCat c) u, 'p' : show (m-n))
                        : prParams tm cs u (n-1) m
...
getVars :: TypeMapping -> [Cat] -> [UserDef] -> [IVar]
getVars tm cs user = reverse $ singleToZero $ foldl addVar [] (map identCat cs)
 where
