...
getCFP cnf (Abs.Grammar defs0) = do
    let rules = inlineDelims rules0
        ...
        srt = let literals           = nub [lit | xs <- map rhsRule rules,
                  ...
                  reservedWords      = nub $ [t | r <- rules, Right (Anonymous t) <- rhsRule r]
                                            ++ [t | r <- rules, Right (Indentation t) <- rhsRule r]
...
delimiterRules a0 l r sep size = [
  ...
  Rule "[]"     (strToCat c)   [Right (Anonymous l)],
  Rule (if optFinal then "(:[])" else
                         "[]")
                (strToCat d)   ([Left a | optFinal] ++ [Right (Anonymous r)]),
  ...
  Rule "[]"     as  [Right (Anonymous l),Right (Anonymous r)] | optEmpty]
 where a = transCat a0
       as = ListCat a
       a' = '@':'@':show a
       c  = '@':'{':show a
       ...
       termin = case sep of
                  Abs.SepSepar t -> [Right (Anonymous t)]
                  Abs.SepTerm  t -> [Right (Anonymous t)]
                  _ -> []
...
separatorRules size c s = if null s then terminatorRules size c s else ifEmpty [
  Rule "(:[])" cs [Left c'],
  Rule "(:)"   cs [Left  c', Right $ Anonymous s, Left  cs]
  ]
 where
   c' = transCat c
   cs = ListCat c'
...
terminatorRules size c s = [
 ...
 where
   c' = transCat c
   cs = ListCat c'
   s' its = if null s then its else Right (Anonymous s) : its
   ifEmpty = if size == Abs.MNonempty
                then Rule "(:[])" cs (Left c' : if null s then [] else [Right $ Anonymous s])
                else Rule "[]" cs []
...
coercionRules (Abs.Ident c) n =
  ...
  [Rule "_" (CoercCat c n)     [Right $ Anonymous "(", Left (Cat c), Right $ Anonymous  ")"]]
...
transItem (Abs.Terminal str) = [Right $ Anonymous  w | w <- words str]
